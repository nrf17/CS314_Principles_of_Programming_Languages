...
static void assign()
{
  int tokVar = (token-'a')*4;
  next_token();
  if(token == '='){
    next_token();
    int val = expr();
    CodeGen(STOREAI, val, 0, tokVar);
  }else{
    printf("Error, no equals symbol after variable.\n");
    exit(EXIT_FAILURE);
  }
}
...
static void print()
{
  ...
  int tokVar = (token-'a')*4;
  if(!is_identifier(token)){
    printf("Error, not printing an identifier\n");
    exit(EXIT_FAILURE);
  }
  CodeGen(OUTPUTAI, 0, tokVar, EMPTY_FIELD);
  next_token();
}
...
static void stmt()
{
  if(is_identifier(token)){
    ...
    printf("Error, can't begin a statment with the current token: %c\n", token);
    exit(EXIT_FAILURE);
  }
...
static void morestmts()
{
  if(token == ';'){
    ...
    printf("Error, improper token, expecting another identifier or period.\n");
    exit(EXIT_FAILURE);
  }
}
